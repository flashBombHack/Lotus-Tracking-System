<template>
    <dashboard-layout>
      ...
      <div class="big-white-card-main">
        <router-link to="/tracking-system"> 
                    <div class="back-card mr-2">
                      <span class="profile-text"><i class="fas fa-arrow-left"></i> Back</span>
                  </div>
                  </router-link><hr>
        ...
        <div class="tab-buttons">
          ...
          >
            <div style="display: flex; white-space: nowrap;"> Awaiting Level(1) Approval
  ...
  <Modal class="custom-modal custom-modal-width" v-show="showReasonModal">
    ...
    computed: {
      ...
      ...mapGetters(['PendingTransactions']),
        filteredRows() {
        let filteredData = [];
        let startIndex = 0;
        let endIndex = 0;
    ...
    methods: {
      ...
      ...mapActions(['fetchPendingTransactions']),
  
      filterTransactions() {
      if (this.selectedTab === 'pending') {
        this.tableData = this.PendingTransactions.filter(transaction => transaction.ApprovalLevel1Status == 0);
      } else {
        this.tableData = this.PendingTransactions.filter(transaction => transaction.ApprovalLevel1Status == 0 && transaction.InvestmentProduct === this.selectedTabName);
      }
